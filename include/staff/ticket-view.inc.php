...
<table width="940" cellpadding="2" cellspacing="0" border="0">
    <tr>
        ...
        <td width="auto" class="flush-right has_bottom_border">
            ...
            <?php
            if(class_exists('TracPlugin')){
	        	echo TracPlugin::tracTicketButton($ticket->getId());
	        }
	        if(class_exists('JIRAPlugin')){
	        	echo JIRAPlugin::ticketButton($ticket->getId());
	        }
            ?>            
...
<?php           if($thisstaff->canBanEmails()) {
            ...
            <?php if($lock && $lock->getStaffId()!=$thisstaff->getId()){ ?>
            	<a id="ticket-unlock" class="action-button" href="tickets.php?id=<?=$id?>&a=unlock"><i class="icon-undo"></i> Unlock</a>
            <?php } ?>
        </td>
    </tr>
</table>
...
<div class="clear"></div>
<h2 style="padding:10px 0 5px 0; font-size:18px;">
Subject: <?php echo Format::htmlchars($ticket->getSubject()); ?>
<?php if(class_exists('TracPlugin')) TracPlugin::GetTicketTracs($ticket->getId()); ?>
<?php if(class_exists('JIRAPlugin')) JIRAPlugin::GetTicket($ticket->getId()); ?>
</h2>
<?php
...
<div id="ticket_thread">
    ...
    if(($thread=$ticket->getThreadEntries($types))) {
       ...
       foreach($thread as $entry) {
	    $offset = '';
    	if($lastMessage>0){
            $response = strtotime($entry['created']);
            $diff = $response-$lastMessage;
            if($diff>0){
        		$diff = abs($diff);
            	...
            	$minutes = floor($diff/(60));
      			$offset = '( in '.($days?$days.($days>1?' days ':' day '):'').($hours?$hours.($hours>0?' hours ':' hour '):'').$minutes.' min )';
        	}else{
        		$offset = '';
        	}
    	}
    	$lastMessage = strtotime($entry['created']);
        ?>
            ...
            <tr>
                ...
                <div>
                    ...
                    <span style="display:inline-block"><?php
                        echo Format::db_datetime($entry['created']);?> <?=$offset?></span>

 ...
 <table class="list" border="0" cellspacing="1" cellpadding="2" width="940">
    <thead>
        <tr>
            ...
            <th width="120">
                 <a <?php echo $resptime_sort; ?> href="tickets.php?sort=resptime&order=<?php echo $negorder; ?><?php echo $qstr; ?>"
                    title="Sort By Response Time <?php echo $negorder; ?>">Response time</a></th>
            <th width="170">
            ...
            <tr id="<?php echo $row['ticket_id']; ?>">
                ...
                <td nowrap>
	             <?php
		            ...
		            $response = strtotime($ticket->getLastResponseDate());
                	$message = strtotime($ticket->getLastMessageDate());
                	$sign ='';
                	if($response>0 && $response>=$message){
                		$diff = $response-$message;
                	}else if(!$ticket->isOpen() && !$response){
                		$diff = 'No response!';
                	}else if(!$ticket->isOpen() && $response){
                		$message = strtotime($ticket->getLastMessageDate());
                		$diff = 'Has response!';
                		...
                		$diff = $message - strtotime(Format::db_datetime(time()));
                		$diff = $message - time();
                		$diff = $message - Misc::dbtime();
                		$sign = '-';
                	}
                	...
                	if(is_numeric($diff)){
	                	...
	                	$minutes = floor($diff/(60));
	          			echo $sign.($days?$days.($days>1?' days ':' day '):'').($hours?$hours.($hours>0?' hours ':' hour '):'').$minutes.' min';
                	}else{
                		echo $diff;
                	}
	             ?>
